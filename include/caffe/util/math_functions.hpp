...
template<typename Dtype>
void caffe_gpu_gemm(const int M, const int N, const int K,
     const Dtype* A, const Dtype* B,   Dtype* C,bool IsAdd_C,bool TransA,bool TransB);

// template<typename Dtype>
// __global__ void MatrixMulKernleTransA(int m, int n, int k, Dtype *A,Dtype  *B, Dtype *C);

// template <typename Dtype>
// void caffe_gpu_gemmTransA(const int m, const int n, const int k,
...
template <typename T>
void kk_gpu_scale(const int N, const T alpha, const T* X, T* Y);
template <typename T>
